...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33601
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33600
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 43601
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 43600
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 33601
	%{_sbindir}/semanage port -d -p tcp 33600
	%{_sbindir}/semanage port -d -p tcp 43601
	%{_sbindir}/semanage port -d -p tcp 43600
	for selinuxvariant in %{selinux_variants}; do
